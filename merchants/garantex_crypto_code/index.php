...
if(!class_exists('merchant_garantex_crypto_code')){
	class merchant_garantex_crypto_code extends Ext_Merchant_Premiumbox {
		...
		function error_back($hash, $code){
			...
			$back = get_request_link('payedmerchant') .'?hash='. is_bid_hash($hash) .'&err=' . $code;
			//	print_r($back); die;
			wp_redirect($back);
			exit;
		}
									...
									if($info){
										...
										if($merch_sum >= $bid_corr_sum or $invalid_minsum > 0){
											if($merch_currency == $bid_currency or $invalid_ctype > 0){
												...
												$trans = $class->get_history_deposits(100);
												foreach($trans as $tran){
													$tran_code = explode(':', trim($tran['address']));	
													$tran_code = trim($tran_code[count($tran_code)-1]);
													if($tran_code){
														if($tran_code == $code ){
															$status = 'realpay';
														}
